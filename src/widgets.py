        ...
        if not recent:
            ...
            for date_str, entry in recent:
                ...
                if entry.end_time:
                    ...
                    duration_str = (
                        f"{hours}h {minutes}m" if minutes > 0 else f"{hours}h"
                    )
                    start_12h = military_to_12hour(entry.start_time)
                    end_12h = military_to_12hour(entry.end_time)
                    time_display = (
                        f"{start_12h}-{end_12h} [cyan]({duration_str})[/cyan]"
                    )
                else:
                    start_12h = military_to_12hour(entry.start_time)
...
class CalendarWidget(Static):
    ...
    def render(self) -> str:
        ...
        lines = []
        lines.append(
            f"[bold cyan]{month_name}[/bold cyan]  (Total: {total_hours:.1f}h)"
        )
        lines.append("")
            ...
            for day in week:
                if day == 0:
                    ...
                    if is_selected:
                        ...
                        week_lines[
                            1
                        ] += f"|[bold green]{day_str:^{cell_width}}[/bold green]"
                    elif is_today:
                        day_str = f"<{day}>"
                        week_lines[
                            1
                        ] += f"|[bold red]{day_str:^{cell_width}}[/bold red]"
                    else:
    ...
    def move_selection(self, delta: int):
        ...
        if entries:
            self.selected_index = max(
                0, min(len(entries) - 1, self.selected_index + delta)
            )
            self.update_display()
